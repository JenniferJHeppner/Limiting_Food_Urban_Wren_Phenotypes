...
### Analysis of Nestling Morphology
# Nestling Mass and Body Condition
...
dat <- read.csv("Data/Nestling_Morphology.csv")
...
dat$time <- as_hms(dat$time) # time of the day blood was taken
...
ggplot(data = dat, aes(x = site, y = mass, colour = trt, group = trt)) +
  ...
  scale_y_continuous(name = 'Fledging Mass (g)', breaks = c(8,9,10,11,12)) +
...
#################################
# Linear Mixed Effect Model of Nestling Mass
mass_best_model <- lmer(mass ~ site * trt + brood_size + julian + year + 
                     (1|nest), data = dat)
...
bartlett.test(resid(mass_best_model), dat$trt)
bartlett.test(resid(mass_best_model), dat$site)
...
# Body Condition all individuals at all ages
...
ggplot(data = dat, aes(x = site, y = bc, colour = trt, group = trt)) +
  ...
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.1, linewidth = 1,
               color = "black", alpha = 0.5, position = position_dodge(width = 0.9)) +
  scale_y_continuous(name = 'Fledging Body Condition', breaks = c(4.5,5,5.5,6)) +
  scale_x_discrete(name = 'Site') +
  labs(color = "Treatment") +
  scale_color_manual(values=c("#999999",  "#008176")) +
  theme_classic() +
...
#################################
# Linear Mixed Effect Model of Nestling Body Condition
bc_best_model <- lmer(bc ~ site * trt + julian + year + brood_size + (1|nest), data = dat)
...
#### Homogeneity of variance ####
bartlett.test(resid(bc_best_model), dat$trt)
bartlett.test(resid(bc_best_model), dat$site)
  # p-value is > 0.05, variance is equal among site and trt groups
